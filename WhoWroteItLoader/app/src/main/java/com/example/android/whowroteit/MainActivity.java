...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Book>{
    ...
    public void searchBooks2(View view){
        String q = mBookInput.getText().toString();
        ...
        // If the network is active and the search field is not empty,
        // add the search term to the arguments Bundle and start the loader.
        if (networkInfo != null && networkInfo.isConnected() && q.length()!=0) {
            mAuthorText.setText("");
            mTitleText.setText(R.string.loading);
            Bundle queryBundle = new Bundle();
            queryBundle.putString("queryString1", q);
    ...
    public Loader<Book> onCreateLoader(int id, Bundle args) {
        return new BookLoader(this, args.getString("queryString1"));
    }
    ...
    public void onLoadFinished(Loader<Book> loader, Book data) {
     ...
     try {
         mTitleText.setText(data.getItems().get(0).getVolumeInfo().getTitle());
         mAuthorText.setText(data.getItems().get(0).getVolumeInfo().getAuthors().get(0));
     }catch (Exception e){
